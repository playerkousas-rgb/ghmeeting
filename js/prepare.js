...
var Prepare={
  ...
  html:function(){
    ...
    var h='<div class="card"><div><h2>🧩 範本</h2><div class="mute" style="font-size:.83rem">'+TPLS.length+' 場即用範本：撳標題睇詳細（流程・道具・帶領），或者直接開帶領。</div></div><div style="margin:8px 0">'+cats.map(function(c){return '<span class="pill'+(Prepare.filter===c[0]?' on':'')+'" onclick="Prepare.f(\''+c[0]+'\')">'+c[1]+'</span>'}).join('')+'</div>';
    ...
    h+='<div class="card"><h2>🗂️ 我嘅集會 <span class="tag g">'+my.length+'</span></h2>'+
      (my.length?'<div class="mute" style="font-size:.83rem">你儲存嘅自製／修改集會。</div>'+my.map(function(m){
        return '<div class="mem tpl-card"><h4>'+esc(m.n)+'</h4>'+'<small class="mute">約 '+m.stages.reduce(function(a,s){return a+(+s.m||0)},0)+' 分鐘・'+m.stages.length+' 節</small>'+'<div class="btns" style="margin:7px 0 0"><button class="btn sm gr" onclick="Lead.startMy(\''+m.id+'\')">▶ 帶領</button></div>'+'<details class="guide-more"><summary>編輯・分享</summary><div class="btns">'+'<button class="btn sm ghost" onclick="Prepare.editMy(\''+m.id+'\')">✏️ 編輯</button>'+'<button class="btn sm ghost" onclick="Prepare.shareMy(\''+m.id+'\')">📤 分享</button>'+'<button class="btn sm ghost rd" style="color:#b71c1c;border-color:#e53935" onclick="Prepare.delMy(\''+m.id+'\')">🗑️ 刪除</button>'+'</div></details></div>'}).join('')
      :'<div class="empty">未有自製集會。揀上面任何範本撳「✏️ 改內容」,或者<br><button class="btn sm" onclick="Prepare.edit(\'blank\')" style="margin-top:8px">➕ 由空白開始</button></div>')+'</div>';
    return h;
  },
